...
#define		MT9V034_ROW_START_DEF			5
...
{
	if (mt9v034->pdata->set_xclk)
		mt9v034->pdata->set_xclk(&mt9v034->subdev, 0);
}
...
static int mt9v034_s_stream(struct v4l2_subdev *subdev, int enable)
{
	...
	struct v4l2_rect *rect = &mt9v034->rect;
	unsigned int hratio;
	unsigned int vratio;
	int ret;
	...
	vratio = DIV_ROUND_CLOSEST(rect->height, format->height);
	...
	ret = mt9v034_write(client, MT9V034_COLUMN_START, rect->left);
	if (ret < 0)
		return ret;

	ret = mt9v034_write(client, MT9V034_ROW_START, rect->top);
	if (ret < 0)
		return ret;

	ret = mt9v034_write(client, MT9V034_WINDOW_WIDTH, rect->width);
	if (ret < 0)
		return ret;

	ret = mt9v034_write(client, MT9V034_WINDOW_HEIGHT, rect->height);
	if (ret < 0)
		return ret;
...
static int mt9v034_registered(struct v4l2_subdev *subdev)
{
	...
	if (data != MT9V034_CHIP_ID) {
		dev_err(&client->dev, "MT9V034 not detected, wrong version "
				"0x%04x\n", data);
		return -ENODEV;
	}
